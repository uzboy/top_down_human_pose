...
class ResNextBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, base_channels=64, groups=32, width_per_group=4, downsample=None):
...
class ResNeXt(NetBase):
    ...
    def make_res_layer(self, in_channels, out_channels, stride, num_blocks, avg_down, groups, width_per_group, base_channels):
        downsample = None
        ...
        layers.append(ResNextBottleneck(in_channels=in_channels, out_channels=out_channels, stride=stride, downsample=downsample,
                                                                                base_channels=base_channels, groups=groups, width_per_group=width_per_group))
...
def get_resnext_50(out_indices=[3]):
    """
        50: (Bottleneck, (3, 4, 6, 3)),
        101: (Bottleneck, (3, 4, 23, 3)),
        152: (Bottleneck, (3, 8, 36, 3))
    """
    return ResNeXt(stage_blocks=[3, 4, 6, 3], out_indices=out_indices, deep_stem=True, avg_down=True)
