...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, se_ratio=16, downsample=None):
...
class ResNet_SE(NetBase):
    ...
    def __init__(self, cfg):
        ...
        _out_channels = cfg.base_channels * Bottleneck.expansion
        for i, num_blocks in enumerate(cfg.stage_blocks):
            ...
            res_layer = self.make_res_layer(num_blocks=num_blocks, in_channels=_in_channels,
                                                                              out_channels=_out_channels, stride=stride, avg_down=cfg.avg_down, se_ratio=cfg.se_ratio)
    ...
    def make_res_layer(self, in_channels, out_channels, stride, num_blocks, avg_down, se_ratio):
        downsample = None
        ...
        layers.append(Bottleneck(in_channels=in_channels, out_channels=out_channels, stride=stride, se_ratio=se_ratio, downsample=downsample))
        in_channels = out_channels

        for _ in range(1, num_blocks):
            layers.append(Bottleneck(in_channels=in_channels, out_channels=out_channels, stride=1, se_ratio=se_ratio))
...
def get_se_resnet_50(out_indices=(3, )):
    ...
    """
    return ResNet_SE(stage_blocks=[3, 4, 6, 3], deep_stem=True, avg_down=True, out_indices=out_indices)
